...
def get_data_type_name(component_name: str, data_type_name: str) -> str:
    ...
    if component_name == "":
        ...
        raise NotImplementedError(
            "$ref expected to start with `#/components/schemas/`, but found `{}` instead for {}".format(
...
def make_object_field(
    go_object_name: str, api_field_name: str, schema: Dict, required: Set[str]
    ...
    if "$ref" in schema:
        ...
        return ObjectField(
            api_name=api_field_name,
            go_type=_parse_referenced_type_name(
                schema, go_object_name + "." + api_field_name
            ),
            description=schema.get("description", ""),
            required=is_required,
        ), []
    else:
        type_name = go_object_name + formatting.snake_case_to_pascal_case(
            api_field_name
        )
        ...
        if schema["type"] in go_primitives:
            ...
            if "items" in schema:
                ...
                if "$ref" in items:
                    ...
                    additional_types.extend(further_types)
                    if item_type.description != "" or not is_primitive_go_type(
                        item_type.go_type
                    ):
