...
class Endpoint:
    ...
    def handler_interface_name(self) -> str:
        if self.operation_id:
            op_id = capitalize_first_letter(self.operation_id)
        else:
            op_id = capitalize_first_letter(self.verb.lower()) + data_types.snake_case_to_pascal_case(self.path.replace('{', '').replace('}', '').replace('/', '_'))
        return op_id

    @property
    def handler_arguments(self) -> str:
        args = [p.name + ' ' + p.go_type for p in self.path_parameters]
        if self.json_request_body_type:
            args.extend(['body {}'.format(self.json_request_body_type), 'bodyParseError *error'])
        return ', '.join(args)
    ...
    for verb in {'get', 'put', 'post', 'delete'}:
        ...
        for security_option in action.get('security', []):
            for scheme, scopes in security_option.items():
                security.schemes[scheme] = security.schemes.get(scheme, AuthorizationOption(required_scopes=scopes))
        responses: List[Response] = []
