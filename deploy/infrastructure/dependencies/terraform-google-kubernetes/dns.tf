...
resource "google_dns_record_set" "external_prometheus" {
  count = var.external_prometheus == "" || var.google_dns_managed_zone_name == "" ? 0 : 1
  name  = "${google_compute_global_address.external_prometheus[0].description}." # description contains the expected hostname
  type  = "A"
  ttl   = 300

  managed_zone = data.google_dns_managed_zone.default[0].name
  rrdatas      = [google_compute_global_address.external_prometheus[0].address]
}
