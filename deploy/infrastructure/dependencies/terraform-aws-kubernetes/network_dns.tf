...
resource "aws_route53_record" "prometheus_hostname_cert_validation" {
  count = var.external_prometheus == "" || var.aws_route53_zone_id == "" ? 0 : length(aws_acm_certificate.prometheus_hostname[0].domain_validation_options)

  allow_overwrite = true
  name            = element(aws_acm_certificate.prometheus_hostname[0].domain_validation_options.*.resource_record_name, count.index)
  type            = element(aws_acm_certificate.prometheus_hostname[0].domain_validation_options.*.resource_record_type, count.index)
  records         = [element(aws_acm_certificate.prometheus_hostname[0].domain_validation_options.*.resource_record_value, count.index)]
  ttl             = 60
  zone_id         = var.aws_route53_zone_id
}
...
resource "aws_route53_record" "external_prometheus" {
  count = var.external_prometheus == "" ? 0 : 1

  zone_id = var.aws_route53_zone_id
  name    = var.external_prometheus
  type    = "A"
  ttl     = 300
  records = [aws_eip.external_prometheus[count.index].public_ip]
}
