...
resource "local_file" "tanka_config_main" {
  content = templatefile("${path.module}/templates/main.jsonnet.tmp", {
    ...
    VAR_ENABLE_SCD             = var.dss_configuration.enable_scd
    ...
    VAR_DESIRED_RID_DB_VERSION = var.dss_configuration.desired_rid_db_version
    VAR_DESIRED_SCD_DB_VERSION = var.dss_configuration.desired_scd_db_version
