...
func TestExecuteUpdateISAValidatesRequest(t *testing.T) {
	for _, r := range []struct {
		name    string
		...
		t.Run(r.name, func(t *testing.T) {
			...
			req := &restapi.UpdateIdentificationServiceAreaRequest{
				Id:      restapi.EntityUUID(uuid.New().String()),
				Version: testdata.Version.String(),
				Body: &restapi.UpdateIdentificationServiceAreaParameters{
					FlightsUrl: restapi.RIDFlightsURL(testdata.CallbackURL),
					Extents:    r.extents,
				},
				Auth: api.AuthorizationResult{ClientID: &testdata.Owner},
			}

			ret, err := executeUpdateISA(ctx, repo, req)
