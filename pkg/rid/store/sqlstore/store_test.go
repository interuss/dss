...
func TestTxnRetrier(t *testing.T) {
	...
	_, err := store.Transact(ctx, "", nil, func(ctx context.Context, repo repos.Repository) error {
...
func TestTransactor(t *testing.T) {
	...
	txnCount := 0
	_, err := store.Transact(ctx, "", nil, func(ctx context.Context, s1 repos.Repository) error {
		// We should get to this retry, then return nothing.
		if txnCount > 0 {
			return errors.New("already failed")
		}
		txnCount++
		_, err := store.Transact(ctx, "", nil, func(ctx context.Context, s2 repos.Repository) error {
