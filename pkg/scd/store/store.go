...
func Init(ctx context.Context, logger *zap.Logger, withCheckCron bool, globalLock bool, hashLock bool) (Store, error) {
	storeType := params.GetStoreParameters().StoreType
	switch storeType {
	case params.SQLStoreType:
		return scdsqlstore.Init(ctx, logger, withCheckCron, globalLock, hashLock)
