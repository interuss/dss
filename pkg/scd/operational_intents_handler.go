...
func (a *Server) CreateOperationalIntentReference(ctx context.Context, req *restapi.CreateOperationalIntentReferenceRequest,
	...
	payload, err := operations.NewCreateOIRPayload(req.Entityid, req.Body, &req.Auth, a.AllowHTTPBaseUrls, timestamp.MustFromContext(ctx))
	if err != nil {
		errResp := &restapi.ErrorResponse{Message: dsserr.Handle(ctx, err)}
...
func (a *Server) UpdateOperationalIntentReference(ctx context.Context, req *restapi.UpdateOperationalIntentReferenceRequest,
	...
	payload, err := operations.NewUpdateOIRPayload(req.Entityid, req.Ovn, req.Body, &req.Auth, a.AllowHTTPBaseUrls, timestamp.MustFromContext(ctx))
	if err != nil {
		errResp := &restapi.ErrorResponse{Message: dsserr.Handle(ctx, err)}
		if stacktrace.GetCode(err) == dsserr.PermissionDenied {
