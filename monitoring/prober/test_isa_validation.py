...
def test_isa_huge_area(session, isa1_uuid):
  ...
  assert resp.json()['message'] == 'bad extents: area is too large (5034048745.815665km² > 2500.000000km²)'
...
def test_isa_missing_footprint(session, isa1_uuid):
  ...
  assert resp.status_code == 400
  assert resp.json()['message'] == 'bad extents: spatial_volume missing required footprint'
