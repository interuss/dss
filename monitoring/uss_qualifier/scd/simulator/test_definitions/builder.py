...
from monitoring.uss_qualifier.scd.data_interfaces import (
    RequiredUSSCapabilities,
    AutomatedTest,
    TestStep,
)


TEST_DEFINITIONS_BASEDIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../", "test_definitions")
)
...
class AutomatedTestBuilder:
    ...
    def build(self) -> AutomatedTest:
        return AutomatedTest(
            name=self.name, uss_capabilities=self.uss_capabilities, steps=self.steps
        )
