...
def execute(
    self, step: TestStep, step_ref: TestStepReference, target: TestTarget
) -> None:
    print(
        "[SCD]     Step: Delete flight {} in {}".format(
            step.delete_flight.flight_name, target.name
        )
    )
    try:
        resp, query = target.delete_flight(step.delete_flight.flight_name)
        self.report_recorder.capture_interaction(
            step_ref, query, "Delete flight as part of test sequence"
        )
    except QueryError as e:
        interaction_id = self.report_recorder.capture_interaction(
            step_ref, e.query, "Delete flight as part of test sequence"
        )
        ...
        raise TestRunnerError(
            "Unsuccessful attempt to delete flight {}".format(step.inject_flight.name),
            issue,
        )
