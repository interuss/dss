...
def _make_api_flight(
    flight: injection_api.TestFlight,
    sp_behavior: behavior.ServiceProviderBehavior,
    dp_behavior: behavior.DisplayProviderBehavior,
    t_earliest: datetime.datetime,
    ...
    most_recent_position: Optional[
        Tuple[datetime.datetime, observation_api.Position]
    ] = None
    ...
    for telemetry in flight.telemetry:
        ...
        if sp_behavior.use_agl_instead_of_wgs84_for_altitude:
            if (
                "height" in telemetry
                and telemetry.height.reference == "TakeoffLocation"
            ):
...
def poll_display_data(dp_id: str) -> Tuple[str, int]:
    ...
    if len(coords) != 4:
        return (
            '"view" argument contains the wrong number of coordinates (expected 4, found {})'.format(
                len(coords)
            ),
            400,
        )
    ...
    if (
        ...
        return (
            '"view" coordinates do not fall within the valid range of -90 <= lat <= 90 and -180 <= lng <= 360',
            400,
        )
    ...
    diagonal = (
        view_min.get_distance(view_max).degrees * geo.EARTH_CIRCUMFERENCE_KM / 360
    )
    if diagonal > 3.6:
        return (
            flask.jsonify(
    ...
    if diagonal <= 1:
        ...
        return flask.jsonify(
            observation_api.GetDisplayDataResponse(
                clusters=clustering.make_clusters(flights, view_min, view_max)
            )
        )
        ...
        for test_id, test in sp.tests.items():
            for flight in test.flights:
                ...
                if tf_details and tf_details.id == flight_id:
                    t_max = max(
                        iso8601.parse_date(telemetry.timestamp)
                        for telemetry in flight.telemetry
                    )
