...
def flights():
    ...
    return (
        flask.jsonify(
            rid.GetFlightsResponse(timestamp=StringBasedDateTime(now), flights=flights)
        ),
        200,
    )
...
def flight_details(id: str):
    ...
    return (
        flask.jsonify(rid.ErrorResponse(message="Flight {} not found".format(id))),
        404,
    )
