...
This page provides information on how to upgrade your CockroachDB version deployed using the
...
If a DSS instance has been deployed with terraform, first upgrade the cluster using [Helm](#helm-deployment)
or [Tanka](#tanka-deployment). Then, update the variable `crdb_image_tag` in your `terraform.tfvars` to
...
`VAR_CRDB_DOCKER_IMAGE_NAME` in TFVARS.gen.md) to ensure the new configuration is aligned with the cluster state.
