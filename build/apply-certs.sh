...
GRAFANA_DASHBOARDS="$DIR/grafana-dashboards"
UPLOAD_CA_KEY=true
...
kubectl delete configmap grafana.dashboards --namespace "$NAMESPACE" --context "$CONTEXT" || true
...
kubectl create configmap grafana.dashboards --namespace "$NAMESPACE" --from-file "$GRAFANA_DASHBOARDS" --context "$CONTEXT"
